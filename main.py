...
### Everything here should be done at start of application
...
def findMatch(event):
    ...
    canvas.create_text(200, 50, text="Your drawing matches: " + match.getLabel(), fill="black", font=('Helvetica 12 bold'))
    canvas.create_text(200, 65, text="Score: " + str(score), fill="black", font=('Helvetica 12 bold'))
...
if (xmlMode == False):
    ...
    # Read in xml templates
    xmlTemplates = readXML("xml")
    # Preprocess xml templates
    processedXMLTemplates = []
    for template in xmlTemplates:
        resampledTemplatePoints = resample(template.getPoints(), 64)
        rotatedTemplatePoints = rotateToZero(resampledTemplatePoints)
        scaledTemplatePoints = scaleToSquare(rotatedTemplatePoints, 150)
        translateToOriginPoints = translateToOrigin(scaledTemplatePoints)
